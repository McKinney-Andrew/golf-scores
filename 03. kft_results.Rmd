...
# load the data
events <- read_csv("kft_events.csv")
archived_results <- read_csv("kft_results_2010_2021.csv") 

```

```{r}

# filter to the current year and get rid of problematic events before extracting event ids
df_01 <-
  events %>%
  filter(year == 2022)
...
write_csv(df_02, "kft_results.csv")
